...
# handles all retry logic consistently
def handle_entrez_request(func_name, operation, max_retries=5, initial_delay=30):
    """Generic handler for Entrez requests with consistent retry logic"""
    retry_count = 0
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count == max_retries:
                sys.stderr.write(f"{func_name}: Failed to read complete response after {max_retries} attempts\n")
                raise
            sys.stderr.write(f"{func_name}: Incomplete read, retrying in {retry_delay} seconds...\n")
            ...
            if e.code in [400, 429, 500, 502, 503, 504]:
                retry_count += 1
                if retry_count == max_retries:
                    sys.stderr.write(f"{func_name}: Server error {e.code} persisted after {max_retries} attempts\n")
                    raise
                sys.stderr.write(f"{func_name}: HTTP Error {e.code}, retrying in {retry_delay} seconds...\n")
                time.sleep(retry_delay)
                retry_delay *= 1.5
            else:
                raise
        except Exception as e:
            sys.stderr.write(f"{func_name}: Unexpected error: {e}\n")
            raise
    
    raise Exception(f"{func_name}: Maximum retries reached")
...
def get_taxonomic_id(taxonomy):
    ...
    def _operation():
        ...
        if 'IdList' in record and record['IdList']:
            ...
            raise ValueError("No taxonomic ID found")
    
    return handle_entrez_request("get_taxonomic_id", _operation)
...
def taxid_exists(taxid):
    ...
    return handle_entrez_request("taxid_exists", _operation)
